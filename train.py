    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if batch_idx==0 and epoch==0:
            for i in range(128):
                ...
                print(input.shape)
                input_0 = input[i].detach().cpu().clone()
                image = tensor2img(input_0[:3])
                image.save('./rotate/'+str(i)+'example.jpg')
                image_gray = tensor2img(input_0[3].unsqueeze(0))
                image_gray.save('./rotate/'+str(i)+'example_depth.jpg')
                print('---------------visualized--')
        ####
