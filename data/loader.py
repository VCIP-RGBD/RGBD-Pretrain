...
def fast_collate(batch):
    ...
    if isinstance(batch[0][0], tuple):
        ...
        batch[0][0]=torchvision.transforms.ToTensor()(batch[0][0])
        targets = torch.tensor([b[1] for b in batch], dtype=torch.int64)
        assert len(targets) == batch_size
        tensor = torch.zeros((batch_size, *batch[0][0].shape), dtype=torch.uint8)
        for i in range(batch_size):
    ...
    def __iter__(self):
        ...
        for next_input, next_target in self.loader:
            ...
            with stream_context():
                ...
                # next_depth = next_depth.to(device=self.device, non_blocking=True)
                
                next_target = next_target.to(device=self.device, non_blocking=True)
                next_input = next_input.to(self.img_dtype).sub_(self.mean).div_(self.std)
                if self.random_erasing is not None:
                    next_input = self.random_erasing(next_input)
                # next_input=ToTensor()(next_input)
                # next_depth = ToTensor()(next_depth)
                # print(next_input.shape,next_depth.shape)
                # next_input = torch.cat([next_input,next_depth],dim=1)
