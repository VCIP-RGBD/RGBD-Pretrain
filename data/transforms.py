...
class ToNumpy:
    ...
    def __call__(self, pil_img, pil_depth):
        np_img = np.array(pil_img, dtype=np.uint8)
        np_depth = np.array(pil_depth, dtype=np.uint8)
        if np_img.ndim < 3:
            np_img = np.expand_dims(np_img, axis=-1)
        np_img = np.rollaxis(np_img, 2)  # HWC to CHW
        return np_img, np_depth
...
class RandomResizedCropAndInterpolation:
    ...
    def __call__(self, img, depth):
        ...
        # print(img.size,self.size,depth.size)
        img_w,img_h=img.size
        depth=F.resize(depth,(img_h,img_w))
        # img=F.resize(img,(img_h,img_w))
        # print(img.size,self.size,(depth.size[1],depth.size[0]))
        return F.resized_crop(img, i, j, h, w, self.size, interpolation), F.resized_crop(depth, i, j, h, w, self.size, interpolation)
