...
def shear_x(img, depth, factor, **kwargs):
    _check_args_tf(kwargs)
    # print(img.size,img.mode,depth.size,depth.mode,color)
    img = img.transform(img.size, Image.AFFINE, (1, factor, 0, 0, 1, 0), **kwargs)
    depth = depth.convert('RGB')
    depth = depth.transform(depth.size, Image.AFFINE, (1, factor, 0, 0, 1, 0), **kwargs)
    depth = depth.convert('L')
    return img, depth

def shear_y(img, depth, factor, **kwargs):
    _check_args_tf(kwargs)
    # print(img.size,img.mode,depth.size,depth.mode,color)
    img=img.transform(img.size, Image.AFFINE, (1, 0, 0, factor, 1, 0), **kwargs)
    depth = depth.convert('RGB')
    depth = depth.transform(depth.size, Image.AFFINE, (1, 0, 0, factor, 1, 0), **kwargs)
    depth = depth.convert('L')
    return img, depth

def translate_x_rel(img,depth, pct, **kwargs):
    pixels = pct * img.size[0]
    _check_args_tf(kwargs)
    return img.transform(img.size, Image.AFFINE, (1, 0, pixels, 0, 1, 0), **kwargs), depth.convert('RGB').transform(depth.size, Image.AFFINE, (1, 0, pixels, 0, 1, 0), **kwargs).convert('L')


def translate_y_rel(img,depth, pct, **kwargs):
    pixels = pct * img.size[1]
    _check_args_tf(kwargs)
    return img.transform(img.size, Image.AFFINE, (1, 0, 0, 0, 1, pixels), **kwargs), depth.convert('RGB').transform(depth.size, Image.AFFINE, (1, 0, 0, 0, 1, pixels), **kwargs).convert('L')
...
def rotate(img,depth, degrees, **kwargs):
    _check_args_tf(kwargs)
    if _PIL_VER >= (5, 2):
        return img.rotate(degrees, **kwargs),depth.convert('RGB').rotate(degrees, **kwargs).convert('L')
    if _PIL_VER >= (5, 0):
        w, h = img.size
        post_trans = (0, 0)
        ...
        return img.transform(img.size, Image.AFFINE, matrix, **kwargs), depth.convert('RGB').transform(depth.size, Image.AFFINE, matrix, **kwargs).convert('L')
    return img.rotate(degrees, resample=kwargs['resample']), depth.convert('RGB').rotate(degrees, resample=kwargs['resample']).convert('L')


def auto_contrast(img,depth, **__):
    return ImageOps.autocontrast(img), depth


def invert(img, depth, **__):
    return ImageOps.invert(img), depth


def equalize(img, depth, **__):
    return ImageOps.equalize(img), depth


def solarize(img,depth, thresh, **__):
    return ImageOps.solarize(img, thresh), depth


def solarize_add(img, depth, add, thresh=128, **__):
    lut = []
    for i in range(256):
        if i < thresh:
    ...
    if img.mode in ("L", "RGB"):
        if img.mode == "RGB" and len(lut) == 256:
            lut = lut + lut + lut
        return img.point(lut),depth

    return img, depth


def posterize(img, depth, bits_to_keep, **__):
    if bits_to_keep >= 8:
        return img
    return ImageOps.posterize(img, bits_to_keep), depth


def contrast(img,depth, factor, **__):
    return ImageEnhance.Contrast(img).enhance(factor), depth


def color(img, depth, factor, **__):
    return ImageEnhance.Color(img).enhance(factor), depth


def brightness(img,depth, factor, **__):
    return ImageEnhance.Brightness(img).enhance(factor), depth


def sharpness(img, depth, factor, **__):
    return ImageEnhance.Sharpness(img).enhance(factor), depth
...
class AugmentOp:
    ...
    def __call__(self, img, depth):
        ...
        return self.aug_fn(img, depth, *level_args, **self.kwargs)
...
def rand_augment_ops(
    ...
    # ['Rotate','AutoContrast','Equalize','Invert','Rotate','PosterizeIncreasing','SolarizeIncreasing','SolarizeAdd','ColorIncreasing','ContrastIncreasing','BrightnessIncreasing','SharpnessIncreasing','ShearX','ShearY','TranslateXRel','TranslateYRel']#,',''Rotate'
    # 'ShearX',
    # 'ShearY',
    # 'TranslateXRel',
    # 'TranslateYRel']#
...
class RandAugment:####
    ...
    def __call__(self, img, depth):
        ...
        # print(self.ops,self.num_layers)
